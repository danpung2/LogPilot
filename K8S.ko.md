...
`./k8s-port-forward.sh` 또는 `kubectl port-forward` 명령어를 실행 중일 때, 다음 로컬 주소로 접근할 수 있습니다:

- **REST API**: `http://localhost:8080/api`
- **gRPC API**: `localhost:50051`
- **Health Check**: `http://localhost:8080/actuator/health` (참고: 포트 포워딩 시 8080 포트를 통해 Management 엔드포인트도 접근 가능할 수 있음, 또는 `logpilot-all` 서비스 포워딩 기준)
- **Metrics**: `http://localhost:8080/actuator/metrics`
- **Prometheus**: `http://localhost:8080/actuator/prometheus`

*주의: minikube(Docker 드라이버) 환경에서는 포트 포워딩이나 터널링 없이는 NodePort나 ClusterIP에 직접 접근할 수 없습니다.*

### API 사용 예시

배포가 정상적으로 완료되었는지 확인하기 위한 간단한 Health Check 예시입니다. 상세한 API 사용법과 요청 필드 정보는 [README.ko.md](README.ko.md) 또는 [README.md](README.md)를 참조하세요.
